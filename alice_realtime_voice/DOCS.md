...
ESP protokolu sabit kalacak, sadece add-on icindeki provider adapter'i degisecek. `0.9.17` ile
Realtime response watchdog eklendi; provider cevap finali donmezse oturum kontrollu kapanir. `0.9.18`
ile Home Assistant erisimi keyword router yerine Realtime tool calling uzerinden yapilir.
`0.9.19` ile HA tool calling icin add-on icinde entity allowlist eklendi. Varsayilan olarak tum
entity'ler kapali gelir; sadece expose edilen entity/domain'ler modele gorunur.
`0.9.21` ile TTS sirasinda soz kesme icin gecmisi silmeyen `cancel_response` komutu eklendi.

`0.9.20` ile allowlist kapsamindaki entity'ler icin `ha_call_service` Realtime tool'u eklendi.
...
- `response_timeout_ms`: Realtime cevap finali gelmezse oturumu toparlama siniri
- `ha_tools_enabled`: HA bridge hazirsa Realtime modele HA tool'larini acar

HA tool calling aktifken model su tool'lari gerektiginde cagirir:

- `ha_conversation`: HA Assist'e dogal dil komutu/sorgusu gonderir
- `ha_search_entities`: Ilgili entity adaylarini kucuk liste olarak arar
- `ha_get_state`: Bilinen entity state'ini okur
- `ha_call_service`: Allowlist icindeki entity icin HA servisi cagirir

Bu sayede tum cihaz listesi her cevapta prompt'a eklenmez; model sadece ihtiyac duydugunda HA'dan veri ister.

HA erisim kapsami add-on ayarlarindan daraltilir:

- `ha_bridge.expose_all_entities`: `true` yapilirsa tum entity'ler acilir; guvenlik icin varsayilan `false`
- `ha_bridge.exposed_entities`: virgullu veya bosluklu entity/glob listesi. Ornek: `weather.home light.salon*`
- `ha_bridge.exposed_domains`: domain bazli acma. Ornek: `weather sensor`
- `ha_bridge.blocked_entities`: acik kapsamin icinden ozellikle kapatilacak entity/glob listesi
- `ha_bridge.allow_conversation_tool`: HA Assist dogal dil tool'unu acar; genis yetki verebildigi icin varsayilan `false`
