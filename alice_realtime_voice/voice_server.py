...
if __name__ == "__main__":
    ...
    cfg: VoiceConfig = app["cfg"]
    _LOG.info(
        "Alice Realtime Voice add-on basliyor. version=%s port=%s realtime=%s realtime_provider=%s realtime_model=%s realtime_turn_detection=%s stt_provider=%s model=%s endpointing=%s endpointing_provider=%s ha_bridge=%s llm=%s tts=%s first_live_test=%s",
        APP_VERSION,
        cfg.port,
