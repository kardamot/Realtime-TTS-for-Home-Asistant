...
EARLY_ABSOLUTE_START_WINDOW_MS = 1200
...
def load_config() -> VoiceConfig:
    ...
    return VoiceConfig(
        ...
        endpointing=EndpointingConfig(
            ...
            speech_end_silence_ms=_read_int(endpointing_raw, "speech_end_silence_ms", 520),
...
class VoiceSession:
    ...
    def _dynamic_start_threshold(self) -> int:
        ...
        delta = max(40, min(240, int(floor * 0.10)))
    ...
    def _dynamic_end_threshold(self) -> int:
        ...
        delta = max(24, min(140, int(floor * 0.06)))
    ...
    async def _process_endpointing(self, chunk: bytes) -> None:
        ...
        if not self._speech_started:
            if self._noise_floor_abs <= 0:
                self._noise_floor_abs = min(level_abs, self.endpointing.end_avg_abs_threshold)
            start_threshold = self._dynamic_start_threshold()

            early_absolute_start = (
                self._received_audio_ms <= EARLY_ABSOLUTE_START_WINDOW_MS
                and level_abs >= self.endpointing.start_avg_abs_threshold
            )
            voice_like = level_abs >= start_threshold or early_absolute_start
            if voice_like:
            ...
            if self._received_audio_ms - self._endpoint_debug_last_ms >= PRE_SPEECH_DEBUG_INTERVAL_MS:
                ...
                _LOG.info(
                    "Voice endpointing pre_speech. session_id=%s audio_ms=%s level_abs=%s raw_avg_abs=%s centered_avg_abs=%s peak_abs=%s noise_floor=%s start_threshold=%s consecutive_ms=%s stripped_headers=%s",
                    self.session_id or "-",
            ...
            if self._consecutive_voice_ms >= self.endpointing.speech_start_min_ms:
                ...
                self._silence_ms = 0
                _LOG.info(
                    "Voice session vad_start. session_id=%s audio_ts=%s level_abs=%s raw_avg_abs=%s centered_avg_abs=%s peak_abs=%s noise_floor=%s threshold=%s",
                    self.session_id or "-",
    ...
    async def handle_start(self, doc: dict) -> None:
        ...
        self.started = True
        _LOG.info(
            "Voice session basladi. session_id=%s sample_rate=%s language=%s endpointing=%s llm=%s tts=%s",
            self.session_id or "-",
            self.sample_rate,
            self.language,
...
def build_app() -> web.Application:
    ...
    try:
        if cfg.stt.provider.strip().lower() == "faster_whisper":
