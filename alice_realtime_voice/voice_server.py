...
class VoiceSession:
    ...
    def _dynamic_start_threshold(self) -> int:
        ...
        return max(
            self.endpointing.start_avg_abs_threshold,
            floor + 80,
            int(floor * 1.35),
        )
    ...
    def _dynamic_end_threshold(self) -> int:
        floor = self._noise_floor_abs
        return max(
            self.endpointing.end_avg_abs_threshold,
            floor + 40,
            int(floor * 1.12),
        )
    ...
    async def _process_endpointing(self, chunk: bytes) -> None:
        ...
        if not self._speech_started:
            self._noise_floor_abs = self._ema_update(self._noise_floor_abs, avg_abs)
            ...
            if (
                ...
                _LOG.info(
                    "Voice session no_speech_timeout. session_id=%s audio_ms=%s avg_abs=%s noise_floor=%s start_threshold=%s",
                    self.session_id or "-",
            ...
            if self._consecutive_voice_ms >= self.endpointing.speech_start_min_ms:
                ...
                self._silence_ms = 0
                _LOG.info(
                    "Voice session vad_start. session_id=%s audio_ts=%s avg_abs=%s noise_floor=%s threshold=%s",
                    self.session_id or "-",
        ...
        if self._silence_ms >= self.endpointing.speech_end_silence_ms:
            ...
            _LOG.info(
                "Voice session vad_end. session_id=%s audio_ts=%s utterance_ms=%s avg_abs=%s noise_floor=%s threshold=%s",
                self.session_id or "-",
                vad_end_ts,
                utterance_ms,
                avg_abs,
    ...
    async def handle_message(self, msg: aiohttp.WSMessage) -> bool:
        if msg.type == aiohttp.WSMsgType.BINARY:
            if not self.started:
                ...
                return True
            chunk = msg.data if isinstance(msg.data, (bytes, bytearray)) else bytes(msg.data)
            if len(chunk) & 1:
                chunk = chunk[:-1]
            if not chunk:
                return True
