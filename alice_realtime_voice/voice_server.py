...
def load_config() -> VoiceConfig:
    ...
    return VoiceConfig(
        ...
        realtime=RealtimeConfig(
            ...
            transcript_wait_ms=_read_int(realtime_raw, "transcript_wait_ms", 250),
...
class VoiceSession:
    ...
    async def _request_realtime_response_after_transcript_wait(self) -> None:
    ...
    async def _request_realtime_response(self) -> None:
        if self._realtime_response_requested or self._realtime_response_done:
            return
    ...
    async def _realtime_response_fallback_after_speech_stop(self) -> None:
        ...
        await self._request_realtime_response_after_transcript_wait()

    async def _finish_realtime_response(self, doc: dict | None = None) -> None:
        ...
        await self.send_event(
            "session_completed",
            reason="openai_realtime_done",
            audio_ms=self._received_audio_ms,
            assistant_text=assistant_text,
            history_messages=len(self._history),
        )
        _LOG.info(
            "OpenAI Realtime oturum tamamlandi. session_id=%s audio_ms=%s transcript_len=%s assistant_len=%s",
            self.session_id or "-",
    ...
    async def _handle_realtime_event(self, doc: dict) -> None:
        ...
        if event_type == "input_audio_buffer.speech_stopped":
            ...
            asyncio.create_task(self._realtime_response_fallback_after_speech_stop())
            return

        if event_type == "input_audio_buffer.committed":
            asyncio.create_task(self._request_realtime_response_after_transcript_wait())
            return
        ...
        if event_type in {"response.output_text.done", "response.text.done", "response.done"}:
...
if __name__ == "__main__":
    ...
    cfg: VoiceConfig = app["cfg"]
    _LOG.info(
        "Alice Realtime Voice add-on basliyor. version=%s port=%s realtime=%s realtime_model=%s realtime_turn_detection=%s stt_provider=%s model=%s endpointing=%s endpointing_provider=%s ha_bridge=%s llm=%s tts=%s first_live_test=%s",
        APP_VERSION,
        cfg.port,
