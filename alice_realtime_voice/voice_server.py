...
def load_config() -> VoiceConfig:
    ...
    return VoiceConfig(
        ...
        endpointing=EndpointingConfig(
            ...
            speech_start_min_ms=_read_int(endpointing_raw, "speech_start_min_ms", 128),
...
class VoiceSession:
    ...
    def _dynamic_start_threshold(self) -> int:
        ...
        delta = max(120, min(600, int(floor * 0.35)))
    ...
    async def _process_endpointing(self, chunk: bytes) -> None:
        ...
        if not self._speech_started:
            ...
            voice_like = (not calibrating and level_abs >= start_threshold) or forced_early_start
            if voice_like:
            ...
            if self._received_audio_ms - self._endpoint_debug_last_ms >= PRE_SPEECH_DEBUG_INTERVAL_MS:
                ...
                _LOG.info(
                    "Voice endpointing pre_speech. session_id=%s audio_ms=%s level_abs=%s raw_avg_abs=%s centered_avg_abs=%s peak_abs=%s noise_floor=%s start_threshold=%s consecutive_ms=%s calibrating=%s forced=%s stripped_headers=%s",
                    self.session_id or "-",
        ...
        if self._received_audio_ms - self._active_debug_last_ms >= POST_SPEECH_DEBUG_INTERVAL_MS:
            ...
            _LOG.info(
                "Voice endpointing active. session_id=%s audio_ms=%s level_abs=%s raw_avg_abs=%s centered_avg_abs=%s peak_abs=%s noise_floor=%s end_threshold=%s silence_ms=%s",
                self.session_id or "-",
                self._received_audio_ms,
                level_abs,
                raw_avg_abs,
