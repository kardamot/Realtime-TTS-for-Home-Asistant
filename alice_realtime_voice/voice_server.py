...
class SileroVadRuntime:
    ...
    def push_pcm16le(self, chunk: bytes) -> list[float]:
        ...
        while len(self._pcm_buffer) >= self.frame_bytes:
            ...
            probs.append(float(self._vad.process(samples.tobytes())))
        return probs
...
class VoiceSession:
    ...
    async def handle_start(self, doc: dict) -> None:
        ...
        self.started = True
        _LOG.info(
            "Voice session basladi. session_id=%s sample_rate=%s language=%s endpointing=%s endpointing_provider=%s llm=%s tts=%s",
            self.session_id or "-",
            self.sample_rate,
            self.language,
...
if __name__ == "__main__":
    ...
    cfg: VoiceConfig = app["cfg"]
    _LOG.info(
        "Alice Realtime Voice add-on basliyor. version=%s port=%s stt_provider=%s model=%s endpointing=%s endpointing_provider=%s ha_bridge=%s llm=%s tts=%s first_live_test=%s",
        APP_VERSION,
        cfg.port,
        cfg.stt.provider,
        cfg.stt.model,
