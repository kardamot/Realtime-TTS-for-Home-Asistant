...
class RealtimeConfig:
    ...
    transcript_wait_ms: int = 800
    response_timeout_ms: int = 12000
    ha_tools_enabled: bool = True
    suppress_empty_transcript_response: bool = True
...
def load_config() -> VoiceConfig:
    ...
    return VoiceConfig(
        ...
        realtime=RealtimeConfig(
            ...
            transcript_wait_ms=_read_int(realtime_raw, "transcript_wait_ms", 800),
            ...
            suppress_empty_transcript_response=_read_bool(realtime_raw, "suppress_empty_transcript_response", True),
...
class VoiceSession:
    ...
    async def handle_cancel_response(self, reason: str = "client_cancel") -> None:
        clean_reason = (reason or "client_cancel").strip() or "client_cancel"
        _LOG.info(
            "Voice session cancel_response. session_id=%s reason=%s realtime=%s response_requested=%s response_done=%s history_messages=%s",
            self.session_id or "-",
            clean_reason,
            self._realtime_ws is not None,
            self._realtime_response_requested,
            self._realtime_response_done,
            len(self._history),
        )
        if self._realtime_ws is not None and not self._realtime_ws.closed:
            try:
                if self._realtime_response_requested and not self._realtime_response_done:
                    await self._send_realtime_json({"type": "response.cancel"})
            except Exception as exc:
                _LOG.debug(
                    "OpenAI Realtime response.cancel gonderilemedi. session_id=%s error=%s",
                    self.session_id or "-",
                    safe_exc_message(exc),
                )
        await self._close_realtime()
    ...
    async def _complete_realtime_empty_transcript(self, reason: str) -> None:
        if self._realtime_response_done:
            return
        self._realtime_response_done = True
        await self._emit_realtime_stt_result_once(reason)
        await self.send_event(
            "session_completed",
            reason=reason,
            audio_ms=self._received_audio_ms,
        ...
        _LOG.info(
            "OpenAI Realtime bos transcript cevabi bastirildi. session_id=%s reason=%s audio_ms=%s speech_started=%s speech_ended=%s",
            self.session_id or "-",
            reason,
    ...
    async def _request_realtime_response_after_transcript_wait(self, generation: int) -> None:
        ...
        if self._realtime_response_requested or self._realtime_response_done:
            return
        if self.realtime.suppress_empty_transcript_response and not self._realtime_transcript.strip():
            await self._complete_realtime_empty_transcript("empty_transcript_suppressed")
            return
    ...
    async def handle_message(self, msg: aiohttp.WSMessage) -> bool:
        ...
        if msg_type in {"cancel", "cancel_response"}:
            await self.handle_cancel_response(str(doc.get("reason") or "client_cancel"))
            return True
        if msg_type == "reset":
            await self._close_realtime()
            self._reset_runtime_state()
