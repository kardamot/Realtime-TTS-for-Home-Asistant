...
- `0.1.202` extends full-frame lab telemetry with separate Mic-0/Mic-1 levels and render correlations plus reference age/gap, TTS underrun, and I2S block timing. JSON/CSV exports use schema v4 while older locally stored rows remain readable.
- `0.1.201` excludes the first 300 ms after the green cue from positive scoring. This prevents an Alice-originated candidate chain that began before the cue from being credited to an unrealistically fast user response.
