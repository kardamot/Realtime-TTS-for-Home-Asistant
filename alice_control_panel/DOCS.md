...
- `0.1.195` enables natural local interruption with ESP-SR 2.4.7 full-duplex AEC: two microphones and the speaker reference are processed with AEC/SE/VAD, automatic delay calibration and residual-echo gating; Alice/Stop remains available as fallback. The FD AEC status distinguishes a live stereo input from an AFE configuration that could not obtain it.
