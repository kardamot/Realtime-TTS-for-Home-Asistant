...
- Version `0.1.202` adds per-microphone level/correlation telemetry and TTS/AEC continuity measurements (reference age/gaps, relay underruns, and I2S blocking) to every full-frame laboratory sample. The panel shows and exports these fields so missed speech can be separated from playback/reference instability.
- Version `0.1.201` adds a 300 ms reaction guard after the green speech cue. Candidate streaks that begin in Alice audio before a human could react are classified as pre-window false candidates, including when the final cut lands just inside the visible green window.
