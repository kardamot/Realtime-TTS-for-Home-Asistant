            ...
            async for msg in self._upstream:
                ...
                if mesaj_turu == "chunk":
                    ...
                    if not self._start_sent:
                        await send_pcm_start(self._ws, DEFAULT_PCM_SAMPLE_RATE, DEFAULT_PCM_CHANNELS)
                        self._pacer = PcmPacer(DEFAULT_PCM_SAMPLE_RATE, DEFAULT_PCM_CHANNELS)
...
async def relay_openai_stream(
    ...
    try:
        async with session.post(OPENAI_SPEECH_URL, headers=headers, json=payload, timeout=timeout) as resp:
            ...
            pacer = PcmPacer(DEFAULT_PCM_SAMPLE_RATE, DEFAULT_PCM_CHANNELS)
...
if __name__ == "__main__":
    ...
    _LOG.info(
        "Alice Realtime TTS add-on basliyor. version=%s port=%s provider=%s pcm_pacing=acik burst_ms=%s",
        ADDON_VERSION,
        cfg.port,
        cfg.provider,
