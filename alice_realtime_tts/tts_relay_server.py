...
def load_config() -> RelayConfig:
    ...
    return RelayConfig(
        ...
        openai_api_key=_read_str(raw, "openai_api_key"),
        openai_model=_read_str(raw, "openai_model", _read_str(raw, "model", "gpt-4o-mini-tts")) or "gpt-4o-mini-tts",
        openai_voice=_read_str(raw, "openai_voice", _read_str(raw, "voice", "coral")) or "coral",
        openai_instructions=_read_str(raw, "openai_instructions", _read_str(raw, "instructions")),
        cartesia_api_key=_read_str(raw, "cartesia_api_key"),
        cartesia_model_id=_read_str(raw, "cartesia_model_id", "sonic-3") or "sonic-3",
        cartesia_voice_id=_read_str(raw, "cartesia_voice_id"),
        cartesia_language=_read_str(raw, "cartesia_language", "tr") or "tr",
        cartesia_version=_read_str(raw, "cartesia_version", "2026-03-01") or "2026-03-01",
        elevenlabs_api_key=_read_str(raw, "elevenlabs_api_key"),
        elevenlabs_model_id=_read_str(raw, "elevenlabs_model_id", "eleven_flash_v2_5") or "eleven_flash_v2_5",
        elevenlabs_voice_id=_read_str(raw, "elevenlabs_voice_id"),
        elevenlabs_output_format=_read_str(raw, "elevenlabs_output_format", "pcm_24000") or "pcm_24000",
        elevenlabs_latency_mode=_read_int(raw, "elevenlabs_latency_mode", 3),
        google_ai_api_key=_read_str(raw, "google_ai_api_key"),
        google_ai_model=_read_str(raw, "google_ai_model", "gemini-3.1-flash-tts-preview") or "gemini-3.1-flash-tts-preview",
        google_ai_voice_name=_read_str(raw, "google_ai_voice_name", "Kore") or "Kore",
        google_ai_prompt_prefix=_read_str(raw, "google_ai_prompt_prefix"),
        google_cloud_credentials_json=_read_str(raw, "google_cloud_credentials_json"),
        google_cloud_voice_name=_read_str(raw, "google_cloud_voice_name"),
        google_cloud_language_code=_read_str(raw, "google_cloud_language_code", "tr-TR") or "tr-TR",
        google_cloud_ssml_gender=_read_str(raw, "google_cloud_ssml_gender", "FEMALE").upper() or "FEMALE",
    ...
    try:
        async with session.post(OPENAI_SPEECH_URL, headers=headers, json=payload, timeout=timeout) as resp:
            ...
            async for chunk in resp.content.iter_chunked(RELAY_CHUNK_BYTES):
                ...
                if cift_len:
                    await ws.send_bytes(bekleyen[:cift_len])
...
async def relay_cartesia_stream(
    ...
    headers = {
        "X-API-Key": cfg.cartesia_api_key,
        "Cartesia-Version": cfg.cartesia_version,
    }
    payload = {
        ...
        async with session.ws_connect(
            ...
            async for msg in upstream:
                ...
                if mesaj_turu == "chunk":
                    ...
                    if pcm:
                        await ws.send_bytes(pcm[: len(pcm) & ~1])
                    continue
...
async def relay_google_ai_stream(
    ...
    try:
        async with session.post(url, params=params, json=payload, timeout=timeout) as resp:
            ...
            await send_pcm_start(ws, DEFAULT_PCM_SAMPLE_RATE, DEFAULT_PCM_CHANNELS)
            for i in range(0, len(pcm), RELAY_CHUNK_BYTES):
                chunk = pcm[i:i + RELAY_CHUNK_BYTES]
                if chunk:
                    await ws.send_bytes(chunk[: len(chunk) & ~1])
            await send_done(ws)
